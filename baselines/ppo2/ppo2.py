...
from baselines.common.tf_util import get_session, get_graph_and_session, save_variables, load_variables
...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        # sess = get_session()
        graph, sess = get_graph_and_session
        ...
        with graph.as_default():
            ...
            self.save = functools.partial(save_variables, sess=sess, graph=graph)
            self.load = functools.partial(load_variables, sess=sess, graph=graph)

            if MPI.COMM_WORLD.Get_rank() == 0:
                initialize(sess=sess)
